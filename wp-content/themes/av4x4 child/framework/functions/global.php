...
===============================================*/
	...
	function st_textdomain() {
		...
		global
			...
			if ( $st_Options['global']['lang'] ) {
				load_theme_textdomain( 'strictthemes', get_stylesheet_directory() . '/assets/lang' ); }
	...
	if ( $st_Options['global']['custom-background'] ) {
		...
		$st_['args'] = array(
			...
			'default-image'			=> !empty( $st_Options['panel']['style']['general']['background-image'] ) ? get_stylesheet_directory_uri() . '/assets/images/' . $st_Options['panel']['style']['general']['background-image'] : '',
	...
	if ( $st_Options['global']['title-tag'] ) {
...
===============================================*/
	...
	function st_favicon() {
		...
		global
			...
			$icon = !empty( $st_Settings['favicon'] ) ? $st_Settings['favicon'] : get_stylesheet_directory_uri() . '/favicon.ico';
	...
	function st_js_data() {
		...
		global
			...
			*/
			...
			stData[2] = "<?php echo get_stylesheet_directory_uri() ?>";
...
===============================================*/
	...
	if ( is_admin() ) {
		...
		function st_welcome_page() {
			include( get_stylesheet_directory() . '/framework/welcome.php' );
		}
		...
		function st_welcome_screen_page() {
			...
			$st_['label'] = sprintf( esc_html__( "%s Setup", 'strictthemes' ), $st_Options['general']['label'] );

			add_theme_page( $st_['label'], $st_['label'], 'edit_theme_options', 'st-theme-welcome', 'st_welcome_page' );
		...
		if ( empty( $st_Settings ) ) {
			...
			$st_['current_theme'] = wp_get_theme();
			$st_['theme_name'] = strtolower( preg_replace( '#[^a-zA-Z]#','', $st_['current_theme']->get( 'Name' ) ) );
			$st_['redirect_check'] = get_option( $st_['theme_name'] . '_after_switch_theme_redirect' );
	
			if ( empty( $st_['redirect_check'] ) ) {
	
				update_option( $st_['theme_name'] . '_after_switch_theme_redirect', 'done' );
...
===============================================*/
	...
	function st_get_redirect_page_url() {
		...
		global
			...
			$go_url = isset( $go_url ) ? $go_url . '?' : get_stylesheet_directory_uri() . '/go.php?';
	...
	function st_logo() {
		...
		if ( function_exists( 'st_kit' ) ) {
			...
			global
				...
				$logo = $logo_type == 'image' && !empty( $st_Settings['logo'] ) ? $st_Settings['logo'] : get_stylesheet_directory_uri() . '/assets/images/logo.png';

				if ( $st_Options['panel']['misc']['hidpi'] && !isset( $st_Settings['hidpi'] ) || !empty( $st_Settings['hidpi'] ) != 'no' ) {
					$logo2x = ' data-hidpi="' . ( !empty( $st_Settings['logo2x'] ) ? esc_url( $st_Settings['logo2x'] ) : get_stylesheet_directory_uri() . '/assets/images/logo2x.png' ) . '"'; }
